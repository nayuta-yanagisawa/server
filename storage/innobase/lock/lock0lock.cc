...
*/
static void wsrep_assert_bf_wait(
	const lock_t* lock_rec1,
	const lock_t* lock_rec2,
	const trx_t* trx1)
{
	if (lock_rec1)
		ut_ad(lock_get_type_low(lock_rec1) == LOCK_REC);
				...
				request is really for a 'gap' type lock */
{
	...
	wsrep_assert_bf_wait(NULL, lock2, trx);
#endif /* WITH_WSREP */
		...
		for (lock_t* lock = lock_rec_get_first_on_page_addr(
			...
			if (lock_get_wait(lock)) {
				...
				if (!conf_lock) {
					...
					wsrep_assert_bf_wait(conf_lock, lock, conf_lock->trx);
#endif /* WITH_WSREP */
				}
			}
		}
...
#endif /* WITH_WSREP */
	return lock_trx_handle_wait_needmutex(trx);
}
